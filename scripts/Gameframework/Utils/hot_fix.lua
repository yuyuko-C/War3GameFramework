...
local log = require 'Gameframework.Base.Log'
local Rsa = require 'Gameframework.Utils.Rsa'

local hot_fix = {}

local ver_name = game.version
local dir_hot_fix = game.Name .. '\\热补丁\\' .. ver_name .. '\\'
...
function hot_fix.main(god_p)
    ...
    if god_p and god_p:IsLocalPlayer() then
        -- 计算热补丁签名
    ...
    for i = 1, 10 do
        local p = god_p or Types.Player[i]
        if p:IsPlayer() then
            p:Sync({ ver = hot_fix.my_ver }, function(data)
                ...
                if count >= Types.Player.CountAlive() then
                    load_hot_fix()
                end
            end)
        else
                ...
                function(data)
                    ...
                    if func then
                        ...
                        if suc then
                            ...
                            Types.Player.GetLocalPlayer():SendMsg(
