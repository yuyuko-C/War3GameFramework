...
local player = require "Gameframework.Types.Player"
...
--	同步完成后回调的函数
function Types.Player:Sync(data, func)
    ...
    local times = 0
    -- 开启计时器,等待同步完成
    Yuyuko.TimeLine:Loop(100, function(t)
        -- 检查是否同步完成
        ...
        for i, name in ipairs(keys) do
            ...
            -- print(('player[%d] synced: %s = %s'):format(self._ID, name, data[name]))
        end
        -- 回调数据
...
--	同步完成时的回调函数
function Types.Player:SyncText(data, func)
    local texts = {}
    ...
    for i = 1, math.ceil(#all_text / 4) do
        ...
        -- 补全长度不足4的情况
        local textLenth = string.len(text)
        for i = 1, 4 - textLenth, 1 do
            text = text .. " "
        end

        local int = Yuyuko.String2ID(text) - 2 ^ 31
    ...
    self:Sync({ count = #ints }, function(data)
        ...
        self:Sync(ints, function(data)
            ...
            for i = text_count * 2 + 2, #data do
                local text = Yuyuko.ID2String(data[i] + 2 ^ 31)
                -- 删除尾部补全的空格
                text = text:gsub("[ \t\n\r]+$", "")
                table.insert(texts, text)
            end
