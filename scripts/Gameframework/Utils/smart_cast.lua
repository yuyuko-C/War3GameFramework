...
local order_id = Yuyuko.ResourceManager.Order2ID
...
local selector = Types.Selector.Create()

local function get_select() return unit.j_unit(message.selection()) end

-- 技能是否开启了智能施法
local function get_smart_cast_type(name)
    local hero = Types.Player.GetLocalPlayer().hero
...
local function can_cast(name)
    ...
    if not skl then return false end
...
local function find_target(skill, x, y)
    -- 筛选出单位
    local target = Types.Point.Create(x, y)
    local g = selector:InRange(target, 200):AddFilter(
            game.target_filter(skill.owner, skill.target_data, true))
        :SortByHeroOrDistance(target):GetSelectUnits()
    return g[1]
end

-- 是否可以发动
local function on_can_order(skill, target)
    if not skill.on_can_order then return true end
...
local function cast_spell(msg, hero, name, force)
    ...
    if not skl then return false end
    -- 不是通魔则返回
    if skl:get_slk 'Order' ~= 'channel' then return false end
    -- 鼠标当前指向的位置
    local x, y = message.mouse()
    local order = skl:get_slk 'DataF1'
    local order_id = order_id[order]
    local flag = 0
    if skl.break_order == 0 then flag = flag + FLAG_RESUME end
    -- print(order, ('%X'):format(order_id))
    if skl.target_type == Types.Ability.TargetType.POINT then
        if (x == 0 and y == 0) or (not force and get_smart_cast_type(name) ~= 1) then
            save_last_skill(msg, hero, name)
            return false
        end
        local target = Types.Point.Create(x, y)
        if not on_can_order(skl, target) then return true end
        clean_last_skill()
        if skl:is_in_range(target) then flag = flag + FLAG_INSTANT end
        message.order_point(order_id, x, y, flag)
        return true
    elseif skl.target_type == Types.Ability.TargetType.NONE then
        clean_last_skill()
        ...
        if target or skl.target_type == Types.Ability.TargetType.UNIT_OR_POINT then
            local target = target or Types.Point.Create(x, y)
            if not on_can_order(skl, target) then return true end
            clean_last_skill()
            if skl:is_in_range(target) then
                flag = flag + FLAG_INSTANT
            end
            message.order_target(order_id, x, y, target and target.handle or 0,
                flag)
            return true
        end
    end
    if get_smart_cast_type(name) == 1 then return true end
    save_last_skill(msg, hero, name)
    return false
end
...
function save_last_skill(msg, hero, name)
    function last_skill(code)
        ...
        if cast_spell(msg, hero, name, true) then return true end
        return false
    end
end
...
function clean_last_skill()
    if not last_skill then return false end
    last_skill = nil
    jass.ForceUICancel()
    return true
end
...
local function is_select_hero()
    ...
    -- 是不是自己的英雄
    if hero and hero == player._self.hero then return hero end
    return false
end
...
local function is_select_shop()
    ...
    -- 是不是自己的英雄
    if shop and shop == player._self.shop then return true end
    return false
end
...
local function is_select_player_unit()
    ...
    -- 是不是自己的英雄
    if u and u:get_owner() == player._self then return true end
    return false
end
...
local function is_select_off_line_hero()
    ...
    if not u:is_hero() then return false end
    if u == player._self.hero then return false end
    local p = u:get_owner()
...
local function lock_hero(flag)
    if flag then
        ...
        jass.SetCameraPosition(jass.GetCameraTargetPositionX(),
...
function message.hook(msg)
    ...
    if msg.type == 'key_down' then
        ...
        for name, key in ipairs { 'Q', 'W', 'E', 'R' } do
            if code == keyboard[key] then
                ...
                if state == 2 then
                    ...
                    -- 按下了ctrl键,学习技能
                    if not can_cast(name) then return false end
                ...
                -- 判断是否是组合键
                if state == 0 then
                    if is_book_ui() then return true end
        ...
        -- 如果是组合键,则跳过
        if state ~= 0 then return true end

        -- 停止键
        if code == keyboard['S'] then
            if not is_select_player_unit() and not is_select_hero() and
                not is_select_off_line_hero() and not select_hero() then
                return true
            end
        ...
        if code == keyboard['A'] then
            if is_select_player_unit() then return true end

            if is_select_hero() then return true end

            if is_select_off_line_hero() then return true end
        ...
        if code == keyboard['B'] then
            ...
            -- 本地发布技能指令
            if cast_spell(msg, hero, '回城') then return false end
        ...
        -- tab
        if code == 515 then
            if game.multiboard then game.multiboard:minimize(false) end
            return false
        end
    end
    ...
    if msg.type == 'key_up' then
        ...
        -- tab
        if code == 515 then
            if game.multiboard then game.multiboard:minimize(true) end
            return false
        end

        if last_skill then last_skill(code) end
    end
    ...
    if msg.type == 'mouse_down' then
        ...
        -- 鼠标左键按下
        if code == 1 then
            if last_skill then return not last_skill() end
            return true
        end

        -- 鼠标右键按下
        if code == 4 then
            if clean_last_skill() then return true end
            if is_select_off_line_hero() then return true end
    ...
    if msg.type == 'mouse_ability' then
        ...
        -- 鼠标左键按下
        if code == 1 then
            if ability == 0 then return true end

            if is_select_shop() then return true end
            ...
            if cast_spell(msg, get_select(), name) then return false end
        end
    end
