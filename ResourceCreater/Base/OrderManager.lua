...
-- 将非系统的命令名称添加到命令列表中用于技能模板生成
for key, value in pairs(Order2ID) do
    if not usedOrder[key] then table.insert(OrderIDList, key) end
end

local usedOrderCount = 0
function mt.GetOrder()
    usedOrderCount = usedOrderCount + 1
    return OrderIDList[usedOrderCount]
end
